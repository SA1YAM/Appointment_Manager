...
export default{
	template: `
		<div>
			...
			<div class = "px-4 mt-4 container-text-center">
				<h1 class = "ms-1 px-2">Edit Appointment</h1>
				<br><br>
				<div id="create-card-form" class="ms-1 row g-3 align-items-center fs-5">
				  ...
				  <div>
					<button @click="EditAppointment" class="btn btn-primary">Submit</button>
				  </div>
	...
	methods: {
		...
		EditAppointment: function(post_id){
			...
			FetchUrl(`http://127.0.0.1:5000/api/appointment/${this.$route.query.appointmentId}`, {
				method: 'put',
				headers: {
...
//			alert(err)
	...
	mounted: function(){
		let date = this.$route.query.scheduled_time.split(" ")
		let dates = date[0].split("/")
		this.formData.date = dates[2] + "-" + dates[1] + "-" + dates[0]
		this.formData.time = date[1]
		console.log(this.formData.date, this.formData.time)
	},
}